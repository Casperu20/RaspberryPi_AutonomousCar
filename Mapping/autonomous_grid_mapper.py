...
def choose_next_grid_target():
    """DFS-style: prefer front, then left, then right unvisited free cell.
    If no such neighbor, pop from the backtrack stack.
    Returns (x, y) target or None."""
    ...
    while position_stack:
        prev = position_stack.pop()
        if prev != (robot_x, robot_y):
            print(f"Backtracking to {prev}")
            return prev
...
def run_grid_mapper():
    ...
    while steps < MAX_GRID_STEPS:
        ...
        if not turn_to_heading(target_heading):
            print("Turn failed. Skipping this target.")
            steps += 1
            continue
        ...
        # 5. Push current position to backtrack stack before moving.
